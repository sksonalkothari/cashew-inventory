...
class RcnSalesHelper:

    @staticmethod
    def consolidate_batches(data: list[dict]) -> dict:
        """Consolidate multiple rcn_sales records into a map by batch_number."""
        ...
        logger.debug(f"Consolidated rcn_sales batch map: {batch_map}")
        return batch_map
    ...
    async def update_batch_quantities(batch_map: dict, headers: dict):
        """Recalculate and update rcn_sales quantity in batch_summary for each batch in batch_map."""
        for batch_number, summary in batch_map.items():
            existing = await batch_summary_service.fetch_batch_by_number(batch_number, headers)
            if existing:
                sales_records = await rcn_sales_dao.fetch_all_rcn_sales_by_batch_number(batch_number, headers)
                total_sales_qty = sum(float(r.get('quantity_kg', 0) or 0) for r in sales_records)

                update_data = {
                    "batch_number": batch_number,
                    "rcn_sales_qty": total_sales_qty,
                    "updated_by": summary.get("created_by")
                }

                logger.info(f"Updating rcn_sales quantity for batch ({batch_number}) to {total_sales_qty}")
                await batch_summary_service.update_batch(update_data, headers)
            else:
                logger.warning(f"No batch_summary found for ({batch_number}) during rcn_sales update step.")

    @staticmethod
    async def update_rcn_sales_quantity_for_batch(batch_number: str, updated_by: str, headers: dict, totals: dict):
        update_data = {"batch_number": batch_number,
                       "rcn_sales_qty": totals.get('total_quantity_kg'),
