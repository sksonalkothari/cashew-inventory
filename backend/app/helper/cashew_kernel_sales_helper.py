    ...
    def consolidate_batches(records: List[dict]) -> Dict[str, dict]:
        ...
        for rec in records:
            ...
            if batch_number not in batch_map:
                batch_map[batch_number] = {"cashew_kernel_sales_qty": 0, "created_by": rec.get('created_by')}
            batch_map[batch_number]["cashew_kernel_sales_qty"] += qty
        return batch_map

    @staticmethod
    async def ensure_batches_exist(batch_map: Dict[str, dict], headers: dict):
        # ensure there is a batch_summary row for each batch_number
        try:
            for batch_number, data in batch_map.items():
                ...
                if not existing:
                    # insert minimal batch_summary record
                    await batch_summary_service.insert_batch_summary({"batch_number": batch_number, "created_by": data.get('created_by')}, headers)
        except Exception as e:
            logger.exception("Failed to ensure batch_summary rows exist for cashew kernel sales")
            raise CashewKernelSalesError(str(e))

    @staticmethod
    async def update_batch_quantities(batch_map: Dict[str, dict], headers: dict):
        try:
            for batch_number in batch_map.keys():
                totals = await cashew_kernel_sales_dao.fetch_all_cashew_kernel_sales_by_batch_number(batch_number, headers)
                total_qty = sum([item.get('quantity_kg') or 0 for item in totals])
                await batch_summary_service.update_batch(batch_number, {"cashew_kernel_sales_qty": total_qty}, headers)
        except Exception as e:
            logger.exception("Failed to update batch_summary cashew_kernel_sales_qty")
            raise CashewKernelSalesError(str(e))
